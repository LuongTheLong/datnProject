...
type EditCategoryProps = {
  defaultValues: AddCategoryFields;
  id: string;
};

export default function EditCategory({ defaultValues, id }: EditCategoryProps) {
  const client = trpc.useContext();
  const { isLoading, mutate } = trpc.useMutation(["category.update-category"], {
    onSuccess: () => {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AddCategoryFields>({ resolver: zodResolver(createCategoryValidator), defaultValues });
